...
class SanitizeChain(Chain):
    ...
    """The keys expected by the SanitizeChains's input dictionary. The output
    dictionary will have the same keys with sanitized_ prepended to the name.
    """
    ...
    def output_keys(self) -> List[str]:
        ...
        """
        return [
            SanitizeChain.convert_key_name(key) for key in self.text_keys
        ] + [self.CONTEXT_KEY]
    ...
    def _call(
        ...
        for i, key in enumerate(self.text_keys):
            outputs[
                SanitizeChain.convert_key_name(key)
            ] = split_sanitized_text[i]
        return outputs
